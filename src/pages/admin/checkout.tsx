...
const CheckOutPage = () => {
  ...
  const processCheckOut = async (qrCode: string) => {
    ...
    try {
      ...
      // OPTIMIZED: Use directCheckOut for faster processing (1 DB call instead of 2)
      console.log('Processing check-out...');
      const result = (await Promise.race([
        api.directCheckOut(qrCode),
        timeoutPromise,
      ...
      if (!result.success) {
        // Not checked in yet, already checked out, or other validation error
      ...
      toast({
        ...
        description: `Terima kasih, ${result.guest.nama}`,
        status: 'success',
        duration: 3000,
        isClosable: true,
        position: 'top',
      ...
      setError(err.message || 'Terjadi kesalahan saat memproses check-out.');
      toast({
        title: 'Gagal',
...
(CheckOutPage as any).getLayout = function getLayout(page: React.ReactElement) {
