...
const BatchWhatsappSender: React.FC<BatchWhatsappSenderProps> = ({
  ...
  const progress = selectedTamu.length > 0 ? ((currentIndex) / selectedTamu.length) * 100 : 0;
  ...
  const handleSend = async () => {
    ...
    const phoneNumber = currentTamu.nomor_hp.replace(/^0/, '62').replace(/\D/g, ''); // Basic formatting
        ...
        <ModalBody>
          {!isFinished ? (
            <VStack spacing={6} align="stretch">
              ...
              >
                <Text fontSize="xs" color="gray.500" mb={1} textTransform="uppercase" fontWeight="bold">
                  Penerima Saat Ini
                </Text>
                <Text fontWeight="bold" fontSize="lg">{currentTamu?.nama}</Text>
                <Text fontSize="sm" fontFamily="monospace">{currentTamu?.nomor_hp}</Text>
                <Badge mt={2} colorScheme={currentTamu?.status_undangan === 'dikirim' ? 'green' : 'yellow'}>
                  Status: {currentTamu?.status_undangan === 'dikirim' ? 'Sudah Dikirim' : 'Belum Dikirim'}
                </Badge>
              </Box>

              <Box>
                <Text fontSize="sm" mb={2} fontWeight="bold">Pesan Preview:</Text>
                <Box 
                  p={3} 
                  bg="green.50" 
                  color="gray.800"
              ...
              <Box>
                 ...
                 <Text fontSize="xs" color="gray.400" mt={1}>
                   Gunakan {'{nama}'}, {'{link}'}, {'{tanggal}'}, {'{jam_mulai}'}, dan {'{jam_selesai}'}.
                 </Text>
              </Box>
            </VStack>
          ) : (
            <VStack spacing={6} py={6} textAlign="center">
              ...
              <Box>
                <Text fontSize="xl" fontWeight="bold" mb={2}>Selesai!</Text>
                <Text color="gray.500">
