...
import type { Permission, PermissionsByModule } from '@/src/domain/services/rbac.service';
            ...
            <Card>
                <CardHeader>
                    <div className="flex justify-between items-start">
                        <div>
                            ...
                            <CardDescription>Manage system permissions and organize them by module</CardDescription>
                        </div>
                ...
                <CardContent>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        ...
                        <Card>
                            <CardHeader className="pb-2">
                                ...
                                <CardTitle className="text-3xl">{filteredPermissions.reduce((sum, m) => sum + m.permissions.length, 0)}</CardTitle>
                            </CardHeader>
                        </Card>
                    </div>
                    ...
                    <div className="relative mb-6">
                        ...
                        <Input placeholder="Search permissions..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" />
                    </div>
                                    ...
                                    <CardHeader>
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <CardTitle className="text-xl capitalize">{moduleGroup.module}</CardTitle>
                                                <Badge variant="secondary">{moduleGroup.permissions.length} permissions</Badge>
                                            </div>
                                        </div>
                                    </CardHeader>
                                    <CardContent>
                                                ...
                                                <TableBody>
                                                    {moduleGroup.permissions.map((permission) => (
                                                        <TableRow key={permission.id}>
                                                            ...
                                                            <TableCell className="text-sm text-muted-foreground">{new Date(permission.created_at).toLocaleDateString()}</TableCell>
                                                            <TableCell className="text-right">
                                                                <div className="flex justify-end gap-2">
                                                                    <Button size="sm" variant="outline" onClick={() => openEditDialog(permission)}>
                                                                        <Edit className="h-4 w-4" />
                                                                    </Button>
                                                                    <Button size="sm" variant="outline" onClick={() => handleDeletePermission(permission.id)}>
                ...
                <DialogContent>
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input id="name" value={permissionName} onChange={(e) => setPermissionName(e.target.value)} placeholder="e.g., users.create" />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="module">Module</Label>
                            <Input id="module" value={permissionModule} onChange={(e) => setPermissionModule(e.target.value)} placeholder="e.g., users" />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="description">Description</Label>
                            <Textarea id="description" value={permissionDescription} onChange={(e) => setPermissionDescription(e.target.value)} placeholder="Describe what this permission allows..." rows={3} />
                        </div>
                    </div>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setIsCreateDialogOpen(false)}>Cancel</Button>
                        <Button onClick={handleCreatePermission} disabled={!permissionName.trim() || !permissionModule.trim()}>Create Permission</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
                ...
                <DialogContent>
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input id="edit-name" value={permissionName} onChange={(e) => setPermissionName(e.target.value)} />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="edit-module">Module</Label>
                            <Input id="edit-module" value={permissionModule} onChange={(e) => setPermissionModule(e.target.value)} />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="edit-description">Description</Label>
                            <Textarea id="edit-description" value={permissionDescription} onChange={(e) => setPermissionDescription(e.target.value)} rows={3} />
                        </div>
                    </div>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setIsEditDialogOpen(false)}>Cancel</Button>
                        <Button onClick={handleEditPermission} disabled={!permissionName.trim() || !permissionModule.trim()}>Save Changes</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </DemoMainLayout>
