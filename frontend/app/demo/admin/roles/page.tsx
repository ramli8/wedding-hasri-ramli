...
import { Plus, Edit, Trash2, Shield, Loader2, CheckCircle, Search, Key, Settings } from 'lucide-react';
import type { Role, RoleWithPermissions, Permission } from '@/src/domain/services/rbac.service';
...
export default function DemoRolesPage() {
    const { rbacService } = useDemoContext();
    ...
    const handleCreateRole = async () => {
        try {
            ...
            await rbacService.createRole({ name: formData.name, description: formData.description || undefined });
    ...
    const handleUpdateRole = async () => {
        ...
        try {
            setIsSubmitting(true);
            await rbacService.updateRole(selectedRole.id, { name: formData.name, description: formData.description || undefined });
    ...
    const handleAssignPermissions = async () => {
        ...
        try {
            setIsSubmitting(true);
            await rbacService.assignPermissionsToRole(selectedRole.id, { permission_ids: selectedPermissions });
            ...
            <Card>
                <CardHeader>
                    <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                        <div>
                            <CardTitle className="text-3xl font-bold flex items-center gap-2">Role Management</CardTitle>
                            <CardDescription className="mt-2">Create and manage roles with specific permissions for access control</CardDescription>
                        </div>
                ...
                <CardContent>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <Card>
                            <CardContent className="flex items-center gap-4 pt-6">
                                <div className="p-3 rounded-full bg-blue-500/20"><Shield className="h-6 w-6 text-blue-500" /></div>
                                <div><p className="text-2xl font-bold">{roles.length}</p><p className="text-sm text-muted-foreground">Total Roles</p></div>
                            </CardContent>
                        </Card>
                        <Card>
                            <CardContent className="flex items-center gap-4 pt-6">
                                <div className="p-3 rounded-full bg-green-500/20"><Key className="h-6 w-6 text-green-500" /></div>
                                <div><p className="text-2xl font-bold">{permissions.length}</p><p className="text-sm text-muted-foreground">Total Permissions</p></div>
                            </CardContent>
                        </Card>
                        <Card>
                            <CardContent className="flex items-center gap-4 pt-6">
                                <div className="p-3 rounded-full bg-purple-500/20"><Settings className="h-6 w-6 text-purple-500" /></div>
                                <div><p className="text-2xl font-bold">{Object.keys(permissionsByModule).length}</p><p className="text-sm text-muted-foreground">Modules</p></div>
                            </CardContent>
                        </Card>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-12">
                            ...
                            <p className="text-muted-foreground">{searchQuery ? 'Try a different search term' : 'Create your first role to get started'}</p>
                        </div>
                    ) : (
                                        ...
                                        <TableRow key={role.id}>
                                            <TableCell>
                                                <div className="flex items-center gap-2">
                                                    <div className="p-2 rounded-full bg-primary/10"><Shield className="h-4 w-4 text-primary" /></div>
                                            ...
                                            <TableCell className="text-right">
                                                <div className="flex justify-end gap-2">
                                                    <Button size="sm" variant="outline" onClick={() => openPermissionsDialog(role)} title="Manage Permissions">
                                                        <Key className="h-4 w-4" />
                                                    </Button>
                                                    <Button size="sm" variant="outline" onClick={() => openEditDialog(role)} disabled={role.is_system} title={role.is_system ? 'System roles cannot be edited' : 'Edit Role'}>
                                                        <Edit className="h-4 w-4" />
                                                    </Button>
                                                    <Button size="sm" variant="outline" onClick={() => openDeleteDialog(role)} disabled={role.is_system} title={role.is_system ? 'System roles cannot be deleted' : 'Delete Role'}>
            ...
            <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Create a new role to define user access levels</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4">
                        <div className="space-y-2">
                            <Label htmlFor="create-name">Role Name</Label>
                            <Input id="create-name" value={formData.name} onChange={(e) => setFormData(prev => ({ ...prev, name: e.target.value }))} placeholder="e.g., Editor, Viewer" />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="create-description">Description</Label>
                            <Textarea id="create-description" value={formData.description} onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))} placeholder="Describe what this role can do..." rows={3} />
                        </div>
                    </div>
                    <DialogFooter>
                ...
                <DialogContent className="max-w-4xl max-h-[85vh]">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            <Key className="h-5 w-5 text-primary" /> Manage Permissions
                        </DialogTitle>
                        <DialogDescription>
                            Assign permissions to {selectedRole?.name}. Selected: {selectedPermissions.length} of {permissions.length} permissions
                        </DialogDescription>
                    </DialogHeader>
                            ...
                            <Tabs defaultValue={Object.keys(permissionsByModule)[0]} className="h-full">
                                <TabsList className="w-full justify-start flex-wrap h-auto gap-1 p-1 bg-muted/50">
                                    {Object.keys(permissionsByModule).map((module) => (
                                        <TabsTrigger key={module} value={module} className="capitalize data-[state=active]:bg-background">
                                            {module}
                                        ...
                                        <TabsContent key={module} value={module} className="mt-4">
                                            <div className="border rounded-lg">
                                                <div className="flex items-center justify-between p-4 border-b bg-muted/30">
                                                    ...
                                                    <Button variant="outline" size="sm" onClick={() => toggleModulePermissions(perms, allSelected)}>
                                                ...
                                                <ScrollArea className="h-[300px]">
                                                    <div className="p-2">
                                                        {perms.map((perm) => (
                                                            <div
                                                                key={perm.id}
                                                                className={`flex items-start space-x-3 p-3 rounded-lg cursor-pointer transition-colors ${selectedPermissions.includes(perm.id) ? 'bg-primary/10 border border-primary/20' : 'hover:bg-muted/50'}`}
                                                                onClick={() => {
                                                            ...
                                                            >
                                                                <input type="checkbox" checked={selectedPermissions.includes(perm.id)} onChange={() => { }} className="h-5 w-5 rounded border-gray-300 mt-0.5" />
                                                                <div className="flex-1">
                                                                    <div className="font-medium">{perm.name}</div>
                                                                    {perm.description && <div className="text-sm text-muted-foreground mt-1">{perm.description}</div>}
                                                                </div>
                                                                {selectedPermissions.includes(perm.id) && <CheckCircle className="h-5 w-5 text-primary" />}
                                                            </div>
                                                        ))}
                                                    </div>
                                                </ScrollArea>
